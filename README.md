...
- Appends ready habit names to `~/notes/temp index.md` once their sampled trigger time has passed.
...
Each run creates or reuses a daily trigger schedule at `.notes_habit_trigger_schedule`.
For each due habit trigger, the script samples a local time from 06:00 through 12:00.
Only triggers whose sampled time has passed are appended to `~/notes/temp index.md`.
